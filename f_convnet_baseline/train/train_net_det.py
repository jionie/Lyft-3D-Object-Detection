...
def train(data_loader, model, optimizer, lr_scheduler, epoch, accumulation_steps=2, logger=None):
...
def main():
    ...
    if cfg.TRAIN.OPTIMIZER == 'adam':
        ...
        optimizer = Ranger(filter(lambda p: p.requires_grad, model.parameters()), cfg.TRAIN.BASE_LR, \
                              weight_decay=cfg.TRAIN.WEIGHT_DECAY)
    elif cfg.TRAIN.OPTIMIZER == 'Radam':
        optimizer = Radam(filter(lambda p: p.requires_grad, model.parameters()), cfg.TRAIN.BASE_LR, \
                              weight_decay=cfg.TRAIN.WEIGHT_DECAY)
    else:
    ...
    if (cfg.TRAIN.LR_POLICY == 'cosine'):
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, cfg.TRAIN.MAX_EPOCH, eta_min=0, last_epoch=-1)
    elif len(LR_STEPS) > 1:
