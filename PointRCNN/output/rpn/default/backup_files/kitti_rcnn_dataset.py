...
class KittiRCNNDataset(KittiDataset):
    def __init__(self, root_dir, npoints=16384, split='train', classes='Car', mode='TRAIN', random_select=True,
        ...
        if cfg.RPN.ENABLED:
            if gt_database_dir is not None:
                ...
                if self.mode == 'TRAIN':
                    self.gt_database = joblib.load(open(gt_database_dir, 'rb'))
                else:
            ...
            for idx in range(0, self.num_sample):
                ...
                if len(obj_list) == 0:
                    # logger.info('No gt classes: %s' % sample_id)
                    continue
    ...
    def preprocess_rpn_training_data(self):
        ...
        for idx in range(0, self.num_sample):
            ...
            obj_list = self.filtrate_objects(self.get_label(sample_id))
            if len(obj_list) == 0:
                # self.logger.info('No gt classes: %s' % sample_id)
                continue
            self.sample_id_list.append(sample_id)
    ...
    def get_image_shape(self, idx):
        return super().get_image_shape(idx)

    def get_calib(self, idx):
        return super().get_calib(idx)

    def get_road_plane(self, idx):
        return super().get_road_plane(idx)

    @staticmethod
    def get_rpn_features(rpn_feature_dir, idx):
        rpn_feature_file = os.path.join(rpn_feature_dir, '%s.npy' % idx)
        rpn_xyz_file = os.path.join(rpn_feature_dir, '%s_xyz.npy' % idx)
        rpn_intensity_file = os.path.join(rpn_feature_dir, '%s_intensity.npy' % idx)
        if cfg.RCNN.USE_SEG_SCORE:
            rpn_seg_file = os.path.join(rpn_feature_dir, '%s_rawscore.npy' % idx)
            rpn_seg_score = np.load(rpn_seg_file).reshape(-1)
            rpn_seg_score = torch.sigmoid(torch.from_numpy(rpn_seg_score)).numpy()
        else:
            rpn_seg_file = os.path.join(rpn_feature_dir, '%s_seg.npy' % idx)
            rpn_seg_score = np.load(rpn_seg_file).reshape(-1)
    ...
    def get_rpn_sample(self, index, aug=False):
        ...
        if self.split == 'train_aug':
            ...
            pts_file = os.path.join(self.aug_pts_dir, '%s.bin' % sample_id)
    ...
    def get_proposal_from_file(self, index):
        ...
        proposal_file = os.path.join(self.rcnn_eval_roi_dir, '%s.txt' % sample_id)
